...
var PostController = module.exports = {
  ...
  sendOne: function(req, res) {
    ...
    if (req.accepts('html')) {
      ...
      res.renderReact('ViewPostPage', { post: req.data.post });
    } else {
      res.json({
        post: req.data.post,
  ...
  sendAll: function(req, res) {
    ...
    var loggedIn = (req.user._id.toString() === req.data.user._id.toString()) ? true : false;
    var title = req.data.title || 'Posts';

    if (req.accepts('html')) {
      res.renderReact('PostListPage', { posts: req.data.posts, user: req.data.user, loggedIn: loggedIn, title: title });
    } else {
  ...
  update: function(req, res, next) {
    ...
    if (req.data.post.isPrivate && !req.body.isPrivate) {
  ...
  showPost: function(req, res, next) {
    if (!req.data.post) return res.status(500).end();
    PostController.redirect('/posts/' + req.data.post.slug)(req, res, next);
  }
